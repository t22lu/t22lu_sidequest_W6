...
export async function loadAssets(levelPkg, tuningDoc) {
  ...
  const playerImg = await loadImageAsync("assets/wolfSpriteSheet.png");
  const boarImg = await loadImageAsync("assets/bearSpriteSheet.png");
  const leafImg = await loadImageAsync("assets/bunnySpriteSheet.png");
...
function defaultBoarAnis() {
  return {
    run: { row: 1, frames: 4, frameDelay: 3 },
    throwPose: { row: 5, frames: 1, frameDelay: Infinity, frame: 0 },
    death: { row: 7, frames: 4, frameDelay: 16 },
  };
}
